...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "mission") then
		...
		elseif player:getStorageValue(Storage.Postman.Mission10) == 3	and player:getStorageValue(Storage.Postman.Rank) == 5 then
			npcHandler:say("There are no missions for you left right now. You already have the title of Arc.Postman.", cid)
			npcHandler.topic[cid] = 0
	...
	elseif msgcontains(msg, "yes") then
		...
		elseif npcHandler.topic[cid] == 27 then
			npcHandler:say("I grant you the title of Arc.Postman. You are a legend in our guild. As privilege of your newly aquired status you are allowed to make use of certain mailboxes in dangerous areas. Just look out for them and you'll see.", cid)
			player:setStorageValue(Storage.Postman.Rank, 5)
			player:setStorageValue(Storage.Postman.Door, 1)
			player:addAchievement('Arc.Postman.)
			npcHandler.topic[cid] = 0
